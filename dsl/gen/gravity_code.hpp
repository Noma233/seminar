...
int kernel(int n, double xi[][3], double xj[][3], double mass[], double ai[][3], double eps2, double g){
   ...
   for (i = 0; i < n; i += 4) {
      ...
      for (j = 0; j < n; j += 1) {
         ...
         x0_tmp = _mm256_sqrt_pd(_mm256_add_pd(eps2_tmp, _mm256_add_pd(_mm256_add_pd(_mm256_mul_pd(dr_tmp_v0, dr_tmp_v0), _mm256_mul_pd(dr_tmp_v1, dr_tmp_v1)), _mm256_mul_pd(dr_tmp_v2, dr_tmp_v2))));
         x1_tmp = _mm256_mul_pd(x0_tmp, _mm256_mul_pd(x0_tmp, x0_tmp));
         ai_tmp_v0 += _mm256_div_pd(_mm256_mul_pd(_mm256_mul_pd(dr_tmp_v0, g_tmp), mass_tmp), x1_tmp);
         ai_tmp_v1 += _mm256_div_pd(_mm256_mul_pd(_mm256_mul_pd(dr_tmp_v1, g_tmp), mass_tmp), x1_tmp);
         ai_tmp_v2 += _mm256_div_pd(_mm256_mul_pd(_mm256_mul_pd(dr_tmp_v2, g_tmp), mass_tmp), x1_tmp);
      };
      ai[i][0] = ai_tmp_v0[0];
