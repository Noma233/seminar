...
void evaluate_gravity_simd(
    ...
    for (i = 0; i < ni; i+=4) {
        ...
        for (j = 0; j < nj; j++) {
            ...
            // __m256d xjv = {j_element[j][0], j_element[j + 1][0], j_element[j + 2][0], j_element[j + 3][0]};
            // __m256d yjv = {j_element[j][1], j_element[j + 1][1], j_element[j + 2][1], j_element[j + 3][1]};
            // __m256d zjv = {j_element[j][2], j_element[j + 1][2], j_element[j + 2][2], j_element[j + 3][2]};
            // __m256d mjv = {j_element[j][3], j_element[j + 1][3], j_element[j + 2][3], j_element[j + 3][3]};
            ...
            __m256d r2v_inv = rv_inv * rv_inv;
            __m256d r3v_inv = r2v_inv * rv_inv;
...
bool check_particles(double axi[], double ayi[], double azi[], double acci[][3], int n) {
    bool flag = true;
    double eps = 0.0001;
    for(int i = 0;i < n;i++) {
        if(abs(axi[i] - acci[i][0]) / acci[i][0] > eps || abs(ayi[i] - acci[i][1]) > eps || abs(azi[i] - acci[i][2]) > eps) {
            flag = false;
        }
    }
    return flag;
...
int main() {
    ...
    clock_t start, end;
    start = clock();
    evaluate_gravity(n, n, id, pos, pos, mj, acci);
    end = clock();
    ...
    start = clock();
    evaluate_gravity_simd(n, n, id, X, Y, Z, X, Y, Z, j_element, mj, axi, ayi, azi);
    end = clock();
    ...
    bool flag = check_particles(axi, ayi, azi, acci, n);
